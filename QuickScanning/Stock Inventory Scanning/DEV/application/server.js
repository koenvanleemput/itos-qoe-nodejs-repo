...
var SERVICE_NAME = "QuickStockScanningMultipleSKU";
var PORT = 8800;
  ...
  app.get("/stock", function (request, response) {
    ...
    Stock.findAll({order: [['id', 'DESC']]}).then(function(stocks) { // find all entries in the barcodes tables
      stocks.forEach(function(stock) {
        dbStocks.push([stock.id,stock.location, stock.inventorynumber,stock.remark,stock.createdAt]); // adds their info to the dbBarcodes value
      });
  ...
  app.post("/stock", function (request, response) {
    ...
    Stock.count({ where: {inventorynumber: request.query.fInventorynumber }}).then(function(c) {
      if (c > 0 && request.query.fInventorynumber !== '') {
        ...
        Stock.create({ location: request.query.fLocation, inventorynumber: request.query.fInventorynumber, remark: request.query.fRemark})
          .then(function(stock) {
