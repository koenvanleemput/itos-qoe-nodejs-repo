...
var main = function() {
  ...
  // TODO extrenal id + tRef aanpassen => extid + '-' + containerno.
  // TODO Item & Client weg => check refs in Code
  ...
  */
  ...
  var createOrderItemJSON = function(data, callback) {
      ...
      var orderItemReq = JSON.stringify({
        ...
        DischargingOrderItems: [
          {
             ExternalId: data.extid + "-" + data.contno,
  ...
  var createLinkJSON = function(data, callback) {
    ...
    let tRef = data.contno + "-" + data.extid;
    let oiRef = data.extid + "-" + data.contno;

    var linkingReq = JSON.stringify({
      TransportReference: tRef,
      OrderExternalIds: [ data.extid ],
      OrderItemExternalIds: [ oiRef ]
    });
  ...
  app.get("/entries/:custcode/:extid/:contno", function (request, response) {
