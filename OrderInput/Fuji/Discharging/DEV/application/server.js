...
// init service
const SERVICE_NAME = "QuickOrderEntryDischarginFuji";
...
var main = function() {
        ...
        goods.push(
          {
            ...
            Sids: [
              ...
              },
              {
                ...
                "Value": parseFloat(value[COL_GRAMMAGE]).toFixed(2)
              },
              {
  ...
  app.get("/maxsequence/:custcode/:extid/:lot", function (request, response) {
    ...
    sequelize.query("select max(oisequence) as seq from entries where custcode = ? and extid = ? and lot = ?",
  ...
  var sequelize = new Sequelize('Fuji_quick_order_entry', DB_USER, DB_PASS, {
    host: '0.0.0.0',
    dialect: 'sqlite',
    logging: SQL_LOGGING,
    pool: {
    ...
    storage: 'data/Fuji_quick_order_entry_' + ENVIRONMENT + '.sqlite'
  });
