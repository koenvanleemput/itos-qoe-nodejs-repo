...
// init service
const SERVICE_NAME = "QuickOrderEntryLoadingExtendedFMS";
...
var main = function() {
  ...
  app.post("/data", function(req,response) {
    ...
    data.packinglist = data.packinglist.filter(function(item, index, array){
  ...
  var createOrderItemJSON = function(data, callback) {
      ...
      data.packinglist.forEach(function(value){
        var unitcode;

        if (value[COL_PL_PRODUCT] === 'FMSROL') {
          unitcode = 'ROLL';
        } else if (value[COL_PL_PRODUCT] === 'FMSPAL') {
          unitcode = 'PAL';
        } else {
          unitcode = 'ERROR';
        }
        ...
        goods.push(
          {
            Status: 'Available',
            ProductCode: value[COL_PL_PRODUCT],
            ItemNumber: value[COL_PL_ITEM_NO],
  ...
  app.get("/entries/:custcode/:extid/:planneddate", function (request, response) {
  ...
  app.post("/entries", function(req,response) {
    ...
    // clean empty rows from table
    d.transportlist = d.transportlist.filter(function(item, index, array){
      if (item[0] !== null && item[0] !== "")
      return item;
    });
    d.plannedgoods = d.plannedgoods.filter(function(item, index, array){
      if (item[0] !== null && item[0] !== "")
      return item;
    });
    d.transportsummary = d.transportsummary.filter(function(item, index, array){
      if (item[0] !== null && item[0] !== "")
      return item;
    });
    d.packinglist = d.packinglist.filter(function(item, index, array){
      if (item[0] !== null && item[0] !== "")
      return item;
    });
    ...
    Entry.count({
      ...
      Entry.create({
        ...
        opcode: d.opcode,
        planneddate: d.planneddate,
        transportlist: JSON.stringify(d.transportlist),
        plannedgoods: JSON.stringify(d.plannedgoods),
        transportsummary: JSON.stringify(d.transportsummary),
        packinglist: JSON.stringify(d.packinglist)
        }).then(function(entry) {
  ...
  //
  app.get("/stockinfo", function(request, response) {
    ...
    // Object.assign(filter, request.query);

    // Get Inventory Numbers from Stock api
    ...
    // set query parms
    req.query(filter);
    // pass through queryparams from client
    req.query(request.url.substr(request.url.indexOf('?') + 1));
    ...
    req.end(function (res) {
      console.log("#####  received stockinfo ####### ");
      // console.log(res);
      response.statusCode = 200;
  ...
  sequelize.authenticate()
    .then(function(err) {
      ...
      Entry = sequelize.define('entries', {
        ...
        transportlist: {type: Sequelize.STRING},
        plannedgoods:  {type: Sequelize.STRING},
        transportsummary: {type: Sequelize.STRING},
        packinglist: {type: Sequelize.STRING}
      });
