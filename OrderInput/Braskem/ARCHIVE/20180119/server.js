...
var main = function() {
  ...
  const COL_CONT = 12;       // Container
  const COL_CONTTYPE = 13;   // Container Type
  const COL_PRODCODE = 14;   // Product Code
  const COL_LOT = 15;        // LOT
  const COL_QTY = 16;        // Qty Bags
  const COL_UNIT = 17;       // Unit
  const COL_COUNTRY = 18;       // Origin Country Code
  const COL_UNITPRICE = 19;  // UnitPrice
  ...
  var createOrderItemJSON = function(container, data, callback) {
      ...
      var orderItemReq = JSON.stringify({
        ...
        ReferenceValues: [
          ...
          {Code: "PCKUPDATE", Value: data.pickup},
          {Code: "SHN", Value: data.shipno},
          {Code: "AGC", Value: data.agentcode},
          {Code: "ARTNUM", Value: data.articleno},
          {Code: "ITNUM", Value: data.itemno},
          {Code: "WHSE", Value: data.warehouse},
        ],
        DischargingOrderItems: [
          {  ExternalId: data.containerTables[container][0][COL_CONT] + "-" + data.extid,
             ...
             ReferenceValues: [
               ...
               {Code: "INVDATE", Value: data.containerTables[container][0][COL_INVDATE]},
