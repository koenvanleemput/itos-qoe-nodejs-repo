...
var loadFormData = function(form, hot) {
  var fdata = {
    ...
    invoicefrom: form.find("input[id='invoice-from']").val(),
    invoicetotal: form.find("input[id='invoice-total']").val(),
...
var retrieveFormData = function(fdata, hot) {
  ...
  $.get(url, {data: JSON.stringify(fdata)}).done(function(response) {
    ...
    if (res === null) {
      ...
      document.getElementById('invoice-from').value  = res.invoicefrom;
      document.getElementById('invoice-total').value  = res.invoicetotal;
