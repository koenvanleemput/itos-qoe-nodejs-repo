...
// init project
const PORT = 8878;
const HOSTNAME = "itos-api.vanmoergroup.com";
const DOMAIN = "itos.vanmoergroup.com";
...
var createTransportJSON = function(container, data, callback) {
  ...
  var transportReq = JSON.stringify({
    ...
    Properties: [
      ...
      { Propertycode: "PCKREF", Value: data.containerTables[container][0][8] },
...
app.get("/entries/:extid", function (request, response) {
  console.log("getting entry " + request.params);
  response.end(JSON.stringify(["hello"]));
});

// save new entry
app.post("/entries", function(req,response) {
  console.log("saving entry " + req.params);
...
sequelize.authenticate()
  .then(function(err) {
    ...
    // define a new table 'input'
    Entry = sequelize.define('entries', {
      location: {type: Sequelize.STRING},
      inventorynumber: {type: Sequelize.STRING},
      remark: {type: Sequelize.STRING}


     // custcode, extid, opcode, planneddate,
     /*
      Container
      Product Code
      LOT
...
function dbSetup(){
  Entry.sync({force: true}) // using 'force' it drops the table barcodes if it already exists, and creates a new one
    .then(function(){
